...
class TestEventSchedulerApp:
    ...
    def test_events_list_empty(self, client):
        ...
        with patch('app.services.event_service.load_events') as mock_load:
    ...
    def test_events_list_with_data(self, client, sample_event):
        ...
        with patch('app.services.event_service.load_events') as mock_load:
            mock_load.return_value = [sample_event]
            response = client.get('/api/events/')
            assert response.status_code == 200
            data = json.loads(response.data)
            assert len(data) == 1
    ...
    def test_create_event_success(self, client, sample_event_data):
        ...
        with patch('app.services.event_service.load_events') as mock_load, \
             patch('app.services.event_service.save_events') as mock_save:
            mock_load.return_value = []
    ...
    def test_service_update_event(self, sample_event):
        ...
        with patch('app.services.event_service.load_events') as mock_load, \
            ...
            updated_event = update_event(sample_event.id, update_data)
    ...
    def test_get_event_by_id_success(self, client, sample_event):
        """Test getting a single event by ID successfully."""
        with patch('app.services.event_service.load_events') as mock_load:
            mock_load.return_value = [sample_event]
